...
# Plot a UMAP at different parameters and present in one figure
...
def plot_umap(X_train):
    ...
    for i, min_dist in enumerate(min_dists):
        for j, n in enumerate(n_neighbors_arr):
            ...
            ax[i][j].set_title(
                f'min_dist: {min_dist} n_neighbors: {n_neighbors}', fontsize=8)
            ax[i][j].tick_params(axis='both', which='both', bottom=False,
